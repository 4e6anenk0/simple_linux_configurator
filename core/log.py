...
_log_format_for_cli = f"%(asctime)s - [%(levelname)s] - %(name)s - (%(filename)s).%(funcName)s(%(lineno)d) - %(message)s"
...
def get_stream_handler():
    ...
    stream_handler.setFormatter(logging.Formatter(_log_format_for_cli))
